...
public class PastConcertController {
    ...
    public PastConcert getPastConcertById(@PathVariable Long concertId) {
        return pastConcertService.getPastConcertById(concertId);
    }
    ...
    public ResponseEntity<List<PastConcert>> getPastConcertsByArtistName(@PathVariable("name") String name) {
        try {
            log.info("Fetching past concerts for artist name: {}", name);
            List<PastConcert> concerts = pastConcertService.getPastConcertsByArtistName(name);
            return ResponseEntity.ok(concerts);
        } catch (Exception e) {
            log.error("Error fetching past concerts for artist: {}", name, e);
